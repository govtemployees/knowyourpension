...
const fields = [
{val:dy, fn:sel(setDy,“dy”), opts:Array.from({length:daysInMonth},(*,i)=>({v:String(i+1).padStart(2,“0”),l:String(i+1)}))},
{val:mo, fn:sel(setMo,“mo”), opts:MONTHS_FULL.map((m,i)=>({v:String(i+1).padStart(2,“0”),l:m}))},
{val:yr, fn:sel(setYr,“yr”), opts:Array.from({length:45},(*,i)=>({v:String(1960+i),l:String(1960+i)}))},
];
return (
<div>
<label style={{ display:“block”, fontSize:11, fontWeight:600, color:T.textDim, marginBottom:4, letterSpacing:0.2 }}>{label}</label>
...
const c={info:{bg:”#eff6ff”,tx:”#1e40af”,ic:“ℹ️”,bd:”#bfdbfe”},warn:{bg:”#fffbeb”,tx:”#92400e”,ic:“⚠️”,bd:”#fcd34d”},ok:{bg:”#f0fdf4”,tx:”#166534”,ic:“✅”,bd:”#86efac”},bad:{bg:”#fef2f2”,tx:”#991b1b”,ic:“🚫”,bd:”#fecaca”}}[type];
return (
...
<button onClick={()=>setScMode(!scMode)} style={{ padding:"7px 13px", borderRadius:100, border:`2px solid ${scMode?"#7c3aed":T.border}`, background:scMode?"#7c3aed":"#ffffff", cursor:"pointer", fontFamily:"inherit", fontSize:12, fontWeight:700, color:scMode?"#ffffff":"#7c3aed", display:"flex", alignItems:"center", gap:4 }}>
...
<div style={{ padding:"10px 14px", borderRadius:10, background:"#f5f3ff", border:"2px solid #7c3aed", marginBottom:12, fontSize:12, color:"#5b21b6", display:"flex", gap:8, alignItems:"center" }}>
<span>⚡</span>
...
{[
{ label:"Profile A Name", val:labelA, set:setLabelA, color:"#1a56a0", bg:"#eff6ff", border:"#bfdbfe" },
{ label:"Profile B Name", val:labelB, set:setLabelB, color:"#7c3aed", bg:"#f5f3ff", border:"#ddd6fe" },
].map((p,i)=>(
<div key={i} style={{ padding:pad, borderRadius:16, background:p.bg, border:`2px solid ${p.border}` }}>
...
style={{ width:"100%", padding:"8px 12px", borderRadius:8, border:`1px solid ${p.border}`, background:"#ffffff", color:T.text, fontSize:14, fontWeight:700, fontFamily:"inherit", boxSizing:"border-box", outline:"none" }}
/>
</div>
...
<th style={{ padding:"10px 12px", textAlign:"right", color:"#1a56a0", fontWeight:700, fontSize:12, borderBottom:`2px solid #bfdbfe`, background:"#eff6ff" }}>{labelA}</th>
<th style={{ padding:"10px 12px", textAlign:"right", color:"#7c3aed", fontWeight:700, fontSize:12, borderBottom:`2px solid #ddd6fe`, background:"#f5f3ff" }}>{labelB}</th>
...
return (
<tr key={i} style={{ borderBottom:`1px solid ${T.border}`, background:i%2===0?"#fafbfc":"#ffffff" }}>
<td style={{ padding:"10px 12px", color:T.textDim, fontWeight:600, fontSize:11 }}>{row.label}</td>
<td style={{ padding:"10px 12px", textAlign:"right", fontWeight:row.highlight?700:500, color:row.highlight?"#1a56a0":T.text, background:"#f8fbff" }}>
{row.isNum ? (row.a ? fmtF(row.a) : "-") : row.a}
</td>
<td style={{ padding:"10px 12px", textAlign:"right", fontWeight:row.highlight?700:500, color:row.highlight?"#7c3aed":T.text, background:"#fbf9ff" }}>
